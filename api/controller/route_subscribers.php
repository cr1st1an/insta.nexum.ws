...
class Route_Subscribers {
    ...
    public function postEmail() {
        ...
        if (empty($response)) {
            $r_getPostParams = $Validator->getPostParams(array('access_token', 'email'));
        ...
        if (empty($response) && !filter_var($post['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            $response['message'] = t('The following email is not valid: ') . $post['email'];
        }

        if (empty($response)) {
            $r_select = $DB_Subscribers->selectFromAccessToken($post['access_token']);
            if ($r_select['success']) {
        ...
        if (empty($response) && $post['email'] !== $subscriber_data['email']) {
            $r_updateEmail = $DB_Subscribers->updateEmail($subscriber_data['id_subscriber'], $post['email']);
            if ($r_updateEmail['success']) {
                getApi()->invoke('/v1/workers/invite_subscriber', EpiRoute::httpGet, array('_GET' => array('id_subscriber' => $subscriber_data['id_subscriber'])));
            } else {
        ...
        if (empty($response)) {
            ...
            $response['message'] = t('Successfully updated the subscriber with id: ') . $subscriber_data['id_subscriber'];
        }
    ...
    public function postInvite() {
        ...
        if (empty($response)) {
            $r_getPostParams = $Validator->getPostParams(array('access_token', 'invite'));
        ...
        if (empty($response)) {
            if (md5($post['invite']) !== $subscriber_data['md5_invite']) {
                $response['success'] = false;
                $response['message'] = t('The access code is not valid.');
                if (empty($subscriber_data['md5_invite']))
                    $response['trigger'] = 'no_email';
            }
        }

        if (empty($response)) {
            $response = $DB_Subscribers->updateVerified($subscriber_data['id_subscriber'], true);
        }
